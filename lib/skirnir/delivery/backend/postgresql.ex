    ...
    def set_unrecent(users_id, mailboxes_id, recent) do
        ...
        case Postgrex.query(@conn, query, [mailboxes_id, users_id, recent]) do
            ...
            {:error, %Postgrex.Error{postgres: %{message: error}}} ->
                Logger.error("[delivery] [uid:#{users_id}] error in mailbox:#{mailboxes_id}: #{error}")
                0
        end
    end
    ...
    def list_mailboxes(users_id, "", wildcard) do
        ...
        case Postgrex.query(@conn, query, [users_id, full_path_like]) do
            ...
            {:ok, %Postgrex.Result{rows: mailboxes}} ->
                {:ok, mailboxes}
            {:error, %Postgrex.Error{postgres: %{message: error}}} ->
                Logger.error("[delivery] [uid:#{users_id}] error listing: #{error}")
                {:error, error}
        end
    end
    ...
    def get_mailbox_info(users_id, full_path) do
        ...
        case result do
            ...
            {:ok, _} ->
                ...
                {:error, :enopath}
            {:error, %Postgrex.Error{postgres: %{message: error}}} ->
                Logger.error("[delivery] [uid:#{users_id}] error in path #{full_path}: #{error}")
                {:error, error}
        end
    end
    ...
    defp get_mailbox_id(users_id, full_path) do
        ...
        case result do
            ...
            {:ok, _} ->
                ...
                nil
            {:error, error} ->
                Logger.error("[delivery] [uid:#{users_id}] #{full_path}: #{inspect(error)}")
                nil
        end
    end
            ...
            parent_id ->
                ...
                case Postgrex.query(@conn, query, args) do
                    ...
                    {:error, %Postgrex.Error{postgres: %{code: :unique_violation}}} ->
                        {:error, :eduplicated}
                    {:error, %Postgrex.Error{postgres: %{message: error}}} ->
                        Logger.error("[delivery] [uid:#{users_id}] creating '#{full_path}': #{error}")
                        {:error, error}
                end
        end
    end
    ...
    def delete_mailbox(users_id, full_path) do
        ...
        case Postgrex.query(@conn, query, [users_id, full_path]) do
            ...
            {:error, %Postgrex.Error{postgres: %{code: :foreign_key_violation}}} ->
                {:error, :enoempty}
            {:error, %Postgrex.Error{postgres: %{message: error}}} ->
                Logger.error("[delivery] [uid:#{users_id}] deleting '#{full_path}': #{error}")
                {:error, error}
        end
    end
